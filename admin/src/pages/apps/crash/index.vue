<script setup>
import { fetchCrashHistory } from '@/services/historyService'
...
const headers = [
  ...
  { title: t('tables.autoCashout'), key: 'autoCashout' },
...
const loadData = async () => {
  try {
    const res = await fetchCrashHistory({
      q: search.value,
    ...
    console.log("✅ Crash API Response:", res.data)

    items.value = res.data.data || []
    total.value = res.data.total || 0
  } catch (err) {
    console.error("❌ Crash fetch error:", err.response?.data || err.message)
    items.value = []
    total.value = 0
  }
}
...
<template>
  <VCard>
    <VCardTitle>{{ t('tables.crashHistory') }}</VCardTitle>
    ...
    >
      ...
      <!-- ⏱️ Auto Cashout -->
      <template #item.autoCashout="{ item }">
        {{ item.raw.autoCashout ? item.raw.autoCashout + 'x' : '—' }}
      </template>
