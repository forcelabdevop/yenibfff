...
const callbackUrlPreview = computed(() => apiBaseUrl + "/payment/meeldev/callback");
...
const saveSettings = async () => {
	...
	try {
		loading.value = true;
		await axios.put("/admin/meeldev/settings", settings.value);
		...
		<VCard class="payment-method-shell">
			<VCardText class="payment-method-header">
				<div>
					...
					<h1 class="payment-method-title">MeelDev</h1>
					<p class="payment-method-subtitle">
						MeelDev ödeme sağlayıcısı için API ve limit ayarlarını yönetin.
					</p>
				</div>
				<VChip
