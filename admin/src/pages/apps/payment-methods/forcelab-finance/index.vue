...
const callbackUrlPreview = computed(() => apiBaseUrl + "/payment/forcelab-finance/callback");
...
const saveSettings = async () => {
	...
	try {
		loading.value = true;
		await axios.put("/admin/forcelab-finance/settings", settings.value);
		...
		<VCard class="payment-method-shell">
			<VCardText class="payment-method-header">
				<div>
					...
					<h1 class="payment-method-title">Forcelab Finance</h1>
					<p class="payment-method-subtitle">
						Forcelab Finance ödeme sağlayıcısı için API ve limit ayarlarını yönetin.
					</p>
				</div>
				<VChip
