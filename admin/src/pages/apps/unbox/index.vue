<script setup>
import { fetchUnboxHistory } from '@/services/historyService'
...
const loadData = async () => {
  try {
    const res = await fetchUnboxHistory({
      q: search.value,
    ...
    console.log("✅ Unbox API Response:", res.data)

    items.value = res.data.data || []
    total.value = res.data.total || 0
  } catch (err) {
    console.error("❌ Unbox fetch error:", err.response?.data || err.message)
    items.value = []
    total.value = 0
  }
}
...
<template>
  <VCard>
    <VCardTitle>{{ t('tables.unboxHistory') }}</VCardTitle>
    ...
    >
      ...
      <template #item.state="{ item }">
        <VChip
          :color="item.raw.state === 'completed' ? 'success' : item.raw.state === 'canceled' ? 'error' : 'info'"
          size="small"
        >
          {{ item.raw.state }}
        </VChip>
