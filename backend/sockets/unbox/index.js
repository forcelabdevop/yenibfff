				...
				try {
					...
					if (
						...
						user = await User.findById(socket.decoded._id)
							.select(
								"username avatar rank wallets currency mute ban"
							)
							.lean();
					}
					...
					error: {
						...
						message:
							err.message !== undefined
								? err.message
								: "You need to slow down, you have send too many requests. Try again in a minute.",
					},
				});
			}
		});
				...
				try {
					...
					try {
						const user = await User.findById(socket.decoded._id)
							.select(
								"username avatar rank wallets currency xp stats limits affiliates anonymous mute ban createdAt"
							)
							.lean();
