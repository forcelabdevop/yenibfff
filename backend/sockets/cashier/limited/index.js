			...
			try {
				...
				if (socket.decoded !== undefined && socket.decoded !== null) {
					user = await User.findById(socket.decoded._id).select(
						"roblox.id username avatar rank proxy mute ban"
					);
				}
			...
			try {
				...
				try {
					const user = await User.findById(socket.decoded._id)
						.select("roblox username avatar rank proxy mute ban")
						.lean();
					socketCheckUserData(user, true);
					settingCheck(user);
					cashierSendLimitedEnableSocket(
		...
		if (socket.decoded !== undefined && socket.decoded !== null) {
			try {
				...
				try {
					const user = await User.findById(socket.decoded._id)
						.select(
							"roblox username avatar rank proxy mute ban verifiedAt"
						)
						.lean();
					socketCheckUserData(user, true);
		...
		if (socket.decoded !== undefined && socket.decoded !== null) {
			try {
				...
				try {
					const user = await User.findById(socket.decoded._id)
						.select(
							"roblox username avatar rank balance limits proxy mute ban verifiedAt"
						)
						.lean();
					socketCheckUserData(user, true);
