				...
				try {
					...
					try {
						const user = await User.findById(socket.decoded._id)
							.select(
								"roblox.id username avatar rank balance xp stats limits mute ban createdAt"
							)
							.lean();
			...
			if (socket.decoded !== undefined && socket.decoded !== null) {
				try {
					...
					try {
						const user = await User.findById(socket.decoded._id)
							.select(
								"roblox.id username avatar rank balance stats limits mute ban"
							)
							.lean();
						socketCheckUserData(user, true);
						settingCheck(user, "games.blackjack.enabled");
			...
			if (socket.decoded !== undefined && socket.decoded !== null) {
				try {
					...
					try {
						const user = await User.findById(socket.decoded._id)
							.select(
								"roblox.id username avatar rank limits mute ban"
							)
							.lean();
						socketCheckUserData(user, true);
						settingCheck(user);
