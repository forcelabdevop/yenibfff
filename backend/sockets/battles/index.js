				...
				try {
					...
					try {
						const user = await User.findById(socket.decoded._id)
							.select(
								"roblox.id username avatar rank balance xp stats limits affiliates anonymous mute ban createdAt"
							)
							.lean();
			...
			if (socket.decoded !== undefined && socket.decoded !== null) {
				try {
					...
					try {
						const user = await User.findById(socket.decoded._id)
							.select(
								"username avatar rank limits anonymous mute ban createdAt"
							)
							.lean();
						socketCheckUserData(user, true);
						settingCheck(user, "games.battles.enabled");
