...
const {
	...
	socketCheckUserSuspension,
} = require("../../utils/socket");
const { settingGet } = require("../../utils/setting");
const { generalGetChatOnlineCount } = require("../../utils/general/chat");
const { requestRemoveProxy } = require("../../utils/request");

// Load controllers
const { generalBetsInit } = require("../../controllers/general/bets");
const { generalChatInit } = require("../../controllers/general/chat");
const {
	generalGetRains,
	generalRainInit,
...
module.exports = (io) => {
		...
		try {
			...
			if (
				...
				try {
					...
					await socketCheckUserSuspension(socket.decoded);
					...
					if (err.code === "ACCOUNT_SUSPENDED") return next(err);
					return next(
						new Error("You need to sign in to perform this action.")
					);
				}
			}
			...
			const generalData = await Promise.all([
				generalGetChatOnlineCount(io),
				generalGetRains(),
			]);

			socket.emit("init", {
				settings: settingGet(),
				online: generalData[0],
				rains: generalData[1],
	...
	io.of("/general").on("connection", (socket) => {
		...
		// console.log("user connected /general");
