...
const upgraderGetItemListSocket = async (io, socket, user, data, callback) => {
	try {
		...
		const sort =
			data.sort === "highest" ? { amountFixed: -1 } : { amountFixed: 1 };

		// Get items and items count from database
		const dataDatabase = await Promise.all([
			LimitedItem.countDocuments({
				name: { $regex: data.search, $options: "i" },
			}),
			LimitedItem.find({ name: { $regex: data.search, $options: "i" } })
				.sort(sort)
				.limit(105)
				.skip(offset)
...
const upgraderSendBetSocket = async (io, socket, user, data, callback) => {
	try {
		...
		const multiplier = Math.floor(
			(Math.floor(data.amountPayout) / amount) * 100
		);
		...
		if (
			(data.mode === "under" &&
				Math.floor((9000 / multiplier) * 1000) >= outcome) ||
			(data.mode === "over" &&
				Math.floor(100000 - (9000 / multiplier) * 1000) <= outcome)
		) {
				...
				user._id,
				{
					$inc: {
						...
						"limits.betToWithdraw":
							Math.floor(user.limits.betToWithdraw - amount) <= 0
								? -user.limits.betToWithdraw
								: -amount,
						"limits.betToRain":
							Math.floor(user.limits.betToRain - amount) <= 0
								? -user.limits.betToRain
								: -amount,
						"leaderboard.points":
							leaderboardDatabase !== null &&
							user.limits.blockSponsor !== true &&
							user.limits.blockLeaderboard !== true
								? amount
								: 0,
