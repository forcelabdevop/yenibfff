		...
		try {
			...
			let dataDatabase = await Promise.all([
				Rain.findOne({
					...
					.lean(),
				Rain.findOne({ state: "running" })
					.select("amount participants creator type state updatedAt")
					.populate({
						path: "creator",
						select: "username avatar rank xp stats anonymous createdAt",
					})
					.lean(),
			]);
			...
			if (dataDatabase[1] !== null && dataDatabase[1].type === "user") {
				dataDatabase[1].creator = generalUserGetFormated(
					dataDatabase[1].creator
				);
			}
...
const generalSendRainCreateSocket = async (
	io,
	socket,
	user,
	data,
	...
	try {
		...
		const rainDatabase = await Rain.findOne({
			type: "user",
			$or: [
				{ state: "created" },
				{ state: "pending" },
		...
		try {
			...
			generalRainCreateBlock.splice(
				generalRainCreateBlock.indexOf(user._id.toString()),
				1
			);
...
const generalSendRainJoinSocket = async (io, socket, user, data, callback) => {
	try {
		...
		let dataDatabase = await Promise.all([
			Rain.findOne({ state: "running" })
				.select("participants type state updatedAt")
				.lean(),
			Rain.find({
		...
		if (dataDatabase[0].type === "site") {
			const transactionsDatabase = await Promise.all([
				RobuxTransaction.find({
					...
					state: "completed",
					createdAt: {
						$gte: new Date().getTime() - 1000 * 60 * 60 * 24 * 14,
					},
				})
					.select("amount deposit state createdAt")
					.lean(),
			...
			generalCheckSendRainJoinTransactions([
				...transactionsDatabase[0],
				...transactionsDatabase[1],
				...transactionsDatabase[2],
				...transactionsDatabase[3],
				...transactionsDatabase[4],
				...transactionsDatabase[5],
			]);
		}

		// Update rain in database
		dataDatabase[0] = await Rain.findByIdAndUpdate(
			dataDatabase[0]._id,
			{
...
const generalRainStart = async (io, rain) => {
	try {
		...
		let rainDatabase = await Rain.findOne({ state: "running" })
		...
		if (rainDatabase !== null) {
			...
			const delay =
				new Date(rainDatabase.updatedAt).getTime() +
				1000 +
				1000 * 60 * 2 -
				new Date().getTime();

			// Update rain in database
			rainDatabase = await Rain.findByIdAndUpdate(
				rain._id,
				{
					state: "pending",
					updatedAt: new Date().getTime(),
			...
			rainDatabase = await Rain.findByIdAndUpdate(
				...
				.populate({
					path: "creator",
					select: "username avatar rank stats anonymous createdAt",
				})
				.lean();

			// If rain is a user created rain format creater user object
			if (rainDatabase.type === "user") {
				rainDatabase.creator = generalUserGetFormated(
					rainDatabase.creator
				);
			}

			// Call rain complete function after a 2 minute delay
			setTimeout(() => {
				generalRainComplete(io, rainDatabase);
			}, 1000 * 60 * 2);
		}
...
const generalRainComplete = async (io, rain) => {
	try {
		...
		let rainDatabase = await Rain.findById(rain._id)
			.select("amount participants creator type state updatedAt")
			.populate({
				path: "participants",
				populate: {
					path: "user",
					select: "balance xp stats rakeback ips mute ban",
				},
			})
			.lean();

		// Get total combined participants xp
		const totalXp = rainDatabase.participants.reduce(
			(a, b) => a + b.user.xp,
			0
		);
		...
		for (const participant of rainDatabase.participants) {
			...
			let payout = Math.floor(
				Math.floor(rainDatabase.amount / 2) /
					rainDatabase.participants.length
			);
			payout =
				payout +
				Math.floor(
					Math.floor(rainDatabase.amount / 2) *
						(participant.user.xp / totalXp)
				);

			if (
				payoutAddresses.includes(participant.user.ips[0].address) !==
					true &&
				isNaN(payout) === false &&
				payout >= 10
			) {
		...
		let dataDatabase = await Promise.all([
			Rain.findByIdAndUpdate(
				rainDatabase._id,
				{ state: "completed", updatedAt: new Date().getTime() },
				{ new: true }
			)
				.select("amount participants state updatedAt")
				.lean(),
			...promisesUsers,
		...
		for (const transaction of dataDatabase.slice(
			promisesUsers.length + 1
		)) {
			io.of("/general")
				.to(transaction.user._id.toString())
				.emit("rainPayout", { transaction: transaction });
		}
...
const generalRainInit = async (io) => {
	try {
		...
		const rainsDatabase = await Rain.find({
			$or: [
				{ state: "created" },
				{ state: "pending" },
				{ state: "running" },
			...
			.select("participants type state createdAt")
			.populate({
				path: "participants",
				populate: { path: "user", select: "balance stats" },
			})
			.lean();

		// Cancel all extisting rains
		for (const rain of rainsDatabase) {
			await Rain.findByIdAndUpdate(
				rain._id,
				{ state: "canceled", updatedAt: new Date().getTime() },
				{}
			);
		}
