...
const blackjackSendJoinSocket = async (io, socket, user, data, callback) => {
	try {
		...
		const blackjackIndexTable = blackjackTables.findIndex(
			(element) => element.table === Math.floor(data.table)
		);
		...
		blackjackCheckSendJoinTable(
			data,
			user,
			blackjackTables[blackjackIndexTable]
		);
		...
		blackjackTables[blackjackIndexTable].players.sort((a, b) => {
			return a.seat - b.seat;
		});

		// Send blackjack table to frontend
		io.of("/blackjack").emit("table", {
			table: blackjackTableSanitize(blackjackTables[blackjackIndexTable]),
		});
...
const blackjackSendBetSocket = async (io, socket, user, data, callback) => {
	try {
		...
		for (const bet of data.bets) {
			const blackjackSeat =
				blackjackTables[blackjackIndexTable].players[
					blackjackTables[blackjackIndexTable].players.findIndex(
						(element) => element.seat === bet.seat
					)
				];
			...
			amountTotal += Math.floor(amountBet);
			...
			if (blackjackSeat.bet === null) {
				...
				promises.push(
					BlackjackBet.findByIdAndUpdate(
						blackjackSeat.bet._id,
						{
							$inc: {
								"amount.main": Math.floor(bet.amount.main),
								"amount.sideLeft": Math.floor(
									bet.amount.sideLeft
								),
								"amount.sideRight": Math.floor(
									bet.amount.sideRight
								),
							},
						},
		...
		blackjackBetPendingCounts[Math.floor(data.table).toString()] += 1;

		// ✅ Settings ve exchangeRates al
		...
		// ✅ Kullanıcının para birimi → USD
		const bettorCurrency = user.currency?.fiatCurrency || "USD";
		const bettorRate = exchangeRates[bettorCurrency] || 1;
		const betInUSD = amountTotal / bettorRate;

		// ✅ Affiliate zinciri
		const fullUser = await User.findById(user._id)
			.select("affiliates.referrer")
			.lean();
		const level1Ref = fullUser?.affiliates?.referrer || null;
		let level2Ref = null;
		let level3Ref = null;
			...
			User.findByIdAndUpdate(
				user._id,
				{
					$inc: {
						...
						"affiliates.generated": amountTotal,
						xp: xpToAdd,
					},
		...
		// ✅ Bets’i tabloya sync et
		for (let bet of dataDatabase.slice(1)) {
			const blackjackIndexSeat = blackjackTables[
				blackjackIndexTable
			].players.findIndex((element) => element.seat === bet.seat);
			if (bet instanceof BlackjackBet === true) {
				bet = bet.toObject();
			}
			blackjackTables[blackjackIndexTable].players[
				blackjackIndexSeat
...
const blackjackSendClearSocket = async (io, socket, user, data, callback) => {
	try {
		...
		let blackjackSeats = blackjackTables[
			blackjackIndexTable
		].players.filter(
			(element) =>
		...
		try {
			...
			blackjackBetPendingCounts[Math.floor(data.table).toString()] =
				blackjackBetPendingCounts[Math.floor(data.table).toString()] +
				1;
			...
			for (const seat of blackjackSeats) {
				...
				promises.push(BlackjackBet.findByIdAndDelete(seat.bet._id, {}));
			}
			...
			for (let seat of blackjackSeats) {
				blackjackTables[blackjackIndexTable].players[
					blackjackTables[blackjackIndexTable].players.findIndex(
						(element) => element.seat === seat.seat
					)
				].bet = null;
			}
...
const blackjackSendInsuranceSocket = async (
	io,
	socket,
	user,
	data,
	...
	try {
		...
		if (data.insurance === true) {
			...
			io.of("/general")
				...
				.emit("user", { user: userDatabase });
		}

		for (let blackjackSeat of blackjackSeats) {
			// Update blackjack table seat bet actions
			blackjackSeat.bet.actions.push(
				data.insurance === true ? "insurance" : "noinsurance"
			);

			// Update seat object in blackjack tablle players array
			blackjackTables[blackjackIndexTable].players.splice(
				blackjackTables[blackjackIndexTable].players.findIndex(
					(element) => element.seat === blackjackSeat.seat
				),
				1,
				blackjackSeat
			);
...
const blackjackSendHitSocket = async (io, socket, user, data, callback) => {
	try {
		...
		let blackjackSeat =
			blackjackTables[blackjackIndexTable].players[
				blackjackTables[blackjackIndexTable].players.findIndex(
					(element) => element.seat === data.seat
				)
			];
		...
		if (blackjackSeat.bet.actions.includes("split") === false) {
			blackjackSeat.bet.cards.push(
				blackjackTables[blackjackIndexTable].game.deck.pop()
			);
		} else if (
			blackjackSeat.bet.actions.includes("stand") === true ||
			blackjackGetCardsValue(blackjackSeat.bet.cardsRight) >= 21
		) {
			blackjackSeat.bet.cardsLeft.push(
				blackjackTables[blackjackIndexTable].game.deck.pop()
			);
		} else {
			blackjackSeat.bet.cardsRight.push(
				blackjackTables[blackjackIndexTable].game.deck.pop()
			);
		}
		...
		blackjackTables[blackjackIndexTable].game.updatedAt =
			new Date().getTime();
	...
	try {
		...
		const dataDatabase = await Promise.all([
			...
			BlackjackBet.findByIdAndUpdate(
				blackjackSeat.bet._id,
				{
					"amount.main": Math.floor(
						blackjackSeat.bet.amount.main * 2
					),
				},
				{}
			),
		...
		blackjackSeat.bet.amount.main = Math.floor(
			blackjackSeat.bet.amount.main * 2
		);
...
const blackjackGameCountdown = (io, blackjackTable) => {
	try {
		// Get blackjack table index from blackjack tables array
		const blackjackIndexTable = blackjackTables.findIndex(
			(element) => element.table === blackjackTable.table
		);
...
const blackjackGameValidate = (io, blackjackTable) => {
	try {
		if (blackjackBetPendingCounts[blackjackTable.table.toString()] <= 0) {
			...
			blackjackTables[blackjackIndexTable].players = blackjackTables[
				blackjackIndexTable
			].players.filter((element) => element.bet !== null);

			// Update blackjack table game state and updated at
			blackjackTables[blackjackIndexTable].game.state =
				blackjackTables[blackjackIndexTable].players.length >= 1
					? "pending"
					: "created";
			blackjackTables[blackjackIndexTable].game.updatedAt =
				new Date().getTime();
			...
			if (blackjackTables[blackjackIndexTable].players.length >= 1) {
				blackjackGameFairness(io, blackjackTables[blackjackIndexTable]);
			}
		} else {
			setTimeout(() => {
				blackjackGameValidate(io, blackjackTable);
			}, 500);
		}
	} catch (err) {
		console.error(err);
...
const blackjackGameFairness = async (io, blackjackTable) => {
	try {
		...
		blackjackTables[blackjackIndexTable].game.fair.seedPublic =
			dataFair.data.head_block_id;
		blackjackTables[blackjackIndexTable].game.fair.blockId =
			dataFair.data.head_block_num;
...
const blackjackGameStart = async (io, blackjackTable) => {
	try {
		...
		const playersCount =
			blackjackTables[blackjackIndexTable].players.length;
			...
			for (let p = 0; p < playersCount + 1; p++) {
				setTimeout(() => {
					...
					if (p < playersCount) {
						...
						if (
							blackjackTables[blackjackIndexTable].game
		...
		setTimeout(() => {
			if (
				blackjackTables[blackjackIndexTable].game.dealerCards[0]
					.rank !== "A" &&
				blackjackGetCardsValue(
					blackjackTables[blackjackIndexTable].game.dealerCards
				) === 21
			) {
				...
				blackjackGameInsurance(
					io,
					blackjackTables[blackjackIndexTable]
				);
			} else {
				blackjackGameRun(io, blackjackTables[blackjackIndexTable], 4);
			}
...
const blackjackGameInsurance = (io, blackjackTable) => {
	try {
		...
		blackjackTables.splice(
			blackjackTables.findIndex(
				(element) => element.table === blackjackTable.table
			),
			1,
			blackjackTable
		...
		// Send blackjack table to frontend
		io.of("/blackjack").emit("table", {
			table: blackjackTableSanitize(blackjackTable),
		});
		...
		let blackjackInsuranceInterval = setInterval(() => {
			...
			if (
				blackjackTable.players.length ===
					blackjackTable.players.filter(
						(element) => element.bet.actions.length === 2
					).length ||
				new Date().getTime() >=
					new Date(blackjackTable.game.updatedAt).getTime() +
						1000 * 10 +
						500
			) {
				...
				for (let i = 0; i < blackjackTable.players.length; i++) {
					if (blackjackTable.players[i].bet.actions.length < 2) {
						blackjackTable.players[i].bet.actions.push(
							"noinsurance"
						);
					}
				}
				...
				if (
					blackjackGetCardsValue(blackjackTable.game.dealerCards) ===
					21
				) {
					blackjackTable.game.state = "completed";
				}
				blackjackTable.game.updatedAt = new Date().getTime();
				...
				if (
					...
					blackjackGameRun(io, blackjackTable, 4);
				}
			}
		}, 500);
	} catch (err) {
	...
	try {
		...
		if (playerIndex === -1) {
			...
			while (
				...
				(blackjackGetCardsValue(dealerCards) === 17 &&
					blackjackCheckCardsSoftSeventeen(dealerCards) === true)
			) {
			...
			setTimeout(() => {
				blackjackGameComplete(io, blackjackTable);
			}, 1000 * (dealerCards.length - 1));
		} else if (
			blackjackTable.players[playerIndex] === undefined ||
			blackjackGetCardsValue(
				blackjackTable.players[playerIndex].bet.cards
			) >= 21
		) {
			blackjackGameRun(io, blackjackTable, playerIndex - 1);
		} else {
			// Update blackjack table player pos and game updated at
			blackjackTable.playersPos =
				blackjackTable.players[playerIndex].seat;
			blackjackTable.game.updatedAt = new Date().getTime();
			...
			let blackjackRunInterval = setInterval(() => {
				...
				if (
					blackjackTable.players[playerIndex].bet.actions.includes(
						"double"
					) === true ||
					(blackjackTable.players[playerIndex].bet.actions.includes(
						"split"
					) === false &&
						(blackjackTable.players[
							playerIndex
						].bet.actions.includes("stand") === true ||
							blackjackGetCardsValue(
								blackjackTable.players[playerIndex].bet.cards
							) >= 21)) ||
					(blackjackTable.players[playerIndex].bet.actions.includes(
						...
						(blackjackTable.players[playerIndex].bet.actions.filter(
							(element) => element === "stand"
						).length >= 2 ||
							(blackjackTable.players[
								playerIndex
							].bet.actions.includes("stand") === true &&
								(blackjackGetCardsValue(
									blackjackTable.players[playerIndex].bet
										.cardsLeft
								) >= 21 ||
									blackjackGetCardsValue(
										blackjackTable.players[playerIndex].bet
					...
					blackjackTable.players[playerIndex].bet.actions.push(
						"stand"
					);
					blackjackTable.game.updatedAt = new Date().getTime();
...
const blackjackGameComplete = async (io, blackjackTable) => {
	try {
		...
		const dealerValue = blackjackGetCardsValue(
			blackjackTables[blackjackIndexTable].game.dealerCards
		);
		...
		for (const player of blackjackTables[blackjackIndexTable].players) {
			...
			if (player.bet.actions.includes("split") === true) {
				...
				let playerLeftValue = blackjackGetCardsValue(
					player.bet.cardsLeft
				);
				let playerRightValue = blackjackGetCardsValue(
					player.bet.cardsRight
				);

				if (playerLeftValue <= 21 && playerLeftValue === dealerValue) {
					amountPayout = player.bet.amount.main;
				} else if (
					playerLeftValue <= 21 &&
					(playerLeftValue > dealerValue || dealerValue > 21)
				) {
					amountPayout = Math.floor(player.bet.amount.main * 2);
				}

				if (
					playerRightValue <= 21 &&
					playerRightValue === dealerValue
				) {
					amountPayout = Math.floor(
						amountPayout + player.bet.amount.main
					);
				} else if (
					playerRightValue <= 21 &&
					(playerRightValue > dealerValue || dealerValue > 21)
				) {
					amountPayout = Math.floor(
						amountPayout + player.bet.amount.main * 2
					);
				}
			} else {
				...
				if (
					...
					if (player.bet.cards.length === 2 && playerValue === 21) {
						amountPayout = player.bet.amount.main;
					} else if (
						player.bet.actions.includes("insurance") === true
					) {
						amountPayout = Math.floor(player.bet.amount.main * 1.5);
					}
				} else {
					if (player.bet.cards.length === 2 && playerValue === 21) {
						amountPayout = Math.floor(player.bet.amount.main * 2.5);
					} else if (
						playerValue <= 21 &&
			...
			const cardsPlayer = player.bet.actions.includes("split")
				? [player.bet.cardsLeft[0], player.bet.cardsRight[0]]
				: player.bet.cards.slice(0, 2);

			// Get side bet multipliers for bet
			const multiplierSideBet = blackjackGetSideBetMultiplier(
				blackjackTables[blackjackIndexTable].game.dealerCards,
				cardsPlayer
			);

			// Get payout amount for player perfect pair side bets
			if (player.bet.amount.sideLeft > 0) {
				amountPayout =
					amountPayout +
					Math.floor(
						player.bet.amount.sideLeft * multiplierSideBet.left
					);
			}

			// Get payout amount for player 21x3 side bets
			if (player.bet.amount.sideRight > 0) {
				amountPayout =
					amountPayout +
					Math.floor(
						player.bet.amount.sideRight * multiplierSideBet.right
					);
			}
			...
			if (player.bet.actions.includes("insurance") === true) {
				amountBet = amountBet + Math.floor(player.bet.amount.main / 2);
			}
			if (player.bet.actions.includes("split") === true) {
				amountBet = amountBet + player.bet.amount.main;
			}
			...
			const amountRakeback =
				player.user.limits.blockSponsor !== true
					? parseFloat(
							(
								amountBet * player.user.rakeback.percentage
							).toFixed(2)
					  )
					: 0;

			const bettorCurrency = player.user.currency?.fiatCurrency || "USD";
			const bettorRate = settings.exchangeRates?.[bettorCurrency] || 1;
			const betInUSD = amountBet / bettorRate;

			const xpToAdd =
				player.user.limits.blockSponsor !== true
					? parseFloat(
							(
								betInUSD * settings.general.reward.multiplier
							).toFixed(2)
					  )
					: 0;
					...
					player.user._id,
					{
						$inc: {
							...
							"limits.betToWithdraw":
								player.user.limits.betToWithdraw <= amountBet
									? -player.user.limits.betToWithdraw
									: -amountBet,
							"limits.betToRain":
								player.user.limits.betToRain <= amountBet
									? -player.user.limits.betToRain
									: -amountBet,
							"leaderboard.points":
								leaderboardDatabase !== null &&
								player.user.limits.blockSponsor !== true &&
								player.user.limits.blockLeaderboard !== true
									? amountBet
									: 0,
			...
			promiseBets.push(
				BlackjackBet.findByIdAndUpdate(
					...
					.select(
						"amount payout multiplier actions user updatedAt createdAt"
					)
					.populate({
						path: "user",
		...
		let dataDatabase = await Promise.all([
			BlackjackGame.findByIdAndUpdate(
				blackjackTables[blackjackIndexTable].game._id,
				{
		...
		for (const user of dataDatabase.slice(1, promiseUsers.length + 1)) {
			io.of("/general")
				.to(user._id.toString())
				.emit("user", { user: user });
		}
		...
		for (const bet of dataDatabase.slice(promiseUsers.length + 1)) {
		...
		setTimeout(async () => {
			// Generate new game
			blackjackTables[blackjackIndexTable].game =
				await blackjackGenerateGame(
					blackjackTables[blackjackIndexTable].table,
					blackjackTables[blackjackIndexTable].game.type
				);
...
const blackjackGameUncomplete = async (io, game) => {
	try {
		...
		for (const bet of game.bets) {
			const amountTotal = Math.floor(
				bet.amount.main + bet.amount.sideLeft + bet.amount.sideRight
			);
		...
		await Promise.all([
			BlackjackGame.findByIdAndUpdate(game._id, {
				state: "canceled",
				updatedAt: new Date().getTime(),
			}),
			...promises,
...
const blackjackInit = async (io) => {
	try {
		...
		const gamesDatabase = await BlackjackGame.find({
			$or: [
				{ state: "created" },
				{ state: "pending" },
				{ state: "running" },
		...
		for (const game of gamesDatabase) {
			blackjackGameUncomplete(io, game);
		}
		...
		for (
			let index = 0;
			index <
			Number(process.env.BLACKJACK_TABLES_STANDARD) +
				Number(process.env.BLACKJACK_TABLES_WHALE);
			index++
		) {
			// Get game type from current index
			const type =
				index < Number(process.env.BLACKJACK_TABLES_STANDARD)
					? "standard"
					: "whale";
