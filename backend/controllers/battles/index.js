...
const battlesGetGameDataSocket = async (io, socket, user, data, callback) => {
	try {
		...
		let battlesGame =
			battlesGames[battlesGetGameIndex(battlesGames, data.gameId)];

		// If battles game was not in battles games array try to get from database
		if (battlesGame === undefined) {
			battlesGame = await BattlesGame.findById(data.gameId)
				.select(
		...
		callback({
			success: true,
			game: battlesSanitizeGame(battlesGame, true),
		});
	} catch (err) {
		callback({
			success: false,
...
const battlesSendCreateSocket = async (io, socket, user, data, callback) => {
	try {
		...
		// Get active boxes from database
		const boxesDatabase = await Box.find({ state: "active" })
			.select("name amount items categories type state")
		...
		const amountUser = Math.floor(
			amount +
				(amount *
					Math.floor(data.playerCount) *
					Math.floor(data.funding)) /
					100
		);
		...
		const betInUSD = amountUser / bettorRate;
...
const battlesSendBotSocket = async (io, socket, user, data, callback) => {
	try {
		...
		try {
			...
			battlesBlockGame.push(
				battlesGames[
					battlesGetGameIndex(battlesGames, data.gameId)
				]._id.toString()
			);
			...
			for (
				...
				if (
					battlesGames[
						battlesGetGameIndex(battlesGames, data.gameId)
					].bets.some((element) => element.slot === i) !== true
				) {
					promises.push(
						BattlesBet.create({
			...
			battlesGames[battlesGetGameIndex(battlesGames, data.gameId)].bets =
				[
					...battlesGames[
						battlesGetGameIndex(battlesGames, data.gameId)
					].bets,
			...
			battlesBlockGame.splice(
				battlesBlockGame.indexOf(
					battlesGames[
						battlesGetGameIndex(battlesGames, data.gameId)
					]._id.toString()
				),
...
const battlesSendJoinSocket = async (io, socket, user, data, callback) => {
	try {
		...
		try {
			...
			const bettorCurrency = user.currency?.fiatCurrency || "USD";
			const bettorRate = exchangeRates[bettorCurrency] || 1;
			const betInUSD = amountGameBet / bettorRate;
			...
			// Remove game id from game block array
			battlesBlockJoin.splice(
				battlesBlockJoin.indexOf(
					battlesGames[
						battlesGetGameIndex(battlesGames, data.gameId)
					]._id.toString()
				),
...
const battlesSendCancelSocket = async (io, socket, user, data, callback) => {
	try {
		...
		try {
			...
			battlesBlockGame.splice(
				battlesBlockGame.indexOf(data.gameId.toString()),
				1
			);
			...
			setTimeout(() => {
				for (let slot = 0; slot < battlesGame.bets.length; slot++) {
					...
					battlesGame.bets[slot].payout =
						battlesGame.bets[slot].payout +
						battlesGetOutcomeItem(round.box.items, outcome)
							.amountFixed;
...
const battlesGameComplete = async (io, battlesGame) => {
	try {
		...
		for (const bet of battlesGame.bets) {
			bet.payout = winnerBets.some(
				(element) => element._id.toString() === bet._id.toString()
			)
				? parseFloat((amountWinTotal / winnerBets.length).toFixed(2))
				: 0;
			...
			if (!bet.bot) {
				...
				// ✅ Affiliate zinciri bul
				const level1Ref = bet.user.affiliates.referrer;
				let level2Ref = null;
				let level3Ref = null;
				...
				// ✅ Kullanıcının bahsi USD’ye çevir
		...
		const dataDatabase = await Promise.all([
			...
			BattlesGame.find({ "options.private": false, state: "completed" })
				...
				.populate({
					...
					populate: {
						path: "user",
						select: "roblox.id username avatar rank xp stats.total anonymous createdAt",
					},
				})
				.lean(),
		]);
