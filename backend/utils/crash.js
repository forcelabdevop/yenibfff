...
const crashCheckSendBetData = (data) => {
    ...
    } else if(Math.floor(data.amount) < Math.floor(process.env.CRASH_MIN_AMOUNT)) {
        throw new Error(`You can only bet a min amount of ₺${parseFloat(process.env.CRASH_MIN_AMOUNT).toFixed(2).replace(/\B(?=(\d{3})+(?!\d))/g, ',')} per game.`);
    } else if(Math.floor(data.amount) > Math.floor(process.env.CRASH_MAX_AMOUNT)) {
        throw new Error(`You can only bet a max amount of ₺${parseFloat(process.env.CRASH_MAX_AMOUNT).toFixed(2).replace(/\B(?=(\d{3})+(?!\d))/g, ',')} per game.`);
    }
}
...
const crashGenerateGame = () => {
    return new Promise(async(resolve, reject) => {
        try {
            ...
            let seedDatabase = await CrashSeed.findOne({ state: 'created' }).sort({ _id: -1 }).lean();

            // Eğer seed bulunamazsa yeni bir seed oluştur
            if (!seedDatabase) {
                seedDatabase = await CrashSeed.create({
            ...
            // Populate kullanımı
            dataDatabase[0] = await CrashGame.findById(dataDatabase[0]._id).populate({ path: 'fair.seed', select: 'seedServer hash' }).lean();
