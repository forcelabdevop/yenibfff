...
const minesCheckSendBetData = (data) => {
    ...
    } else if(Math.floor(data.amount) < Math.floor(process.env.MINES_MIN_AMOUNT)) {
        throw new Error(`You can only bet a min amount of ₺${parseFloat(process.env.MINES_MIN_AMOUNT).toFixed(2).replace(/\B(?=(\d{3})+(?!\d))/g, ',')} per game.`);
    } else if(Math.floor(data.amount) > Math.floor(process.env.MINES_MAX_AMOUNT)) {
        throw new Error(`You can only bet a max amount of ₺${parseFloat(process.env.MINES_MAX_AMOUNT).toFixed(2).replace(/\B(?=(\d{3})+(?!\d))/g, ',')} per game.`);
    }
}
...
const minesGetGamePayout = (minesGame) => {
    ...
    if (minesGame.revealed.length >= 1) {
        const first = 25 === minesGame.revealed.length
            ? 1
            : minesGetGameFactorial(25) /
              (minesGetGameFactorial(minesGame.revealed.length) *
               minesGetGameFactorial(25 - minesGame.revealed.length));

        const second = (25 - minesGame.minesCount) === minesGame.revealed.length
            ? 1
            : minesGetGameFactorial(25 - minesGame.minesCount) /
              (minesGetGameFactorial(minesGame.revealed.length) *
               minesGetGameFactorial((25 - minesGame.minesCount) - minesGame.revealed.length));
    ...
    const payout = minesGame.amount * multiplier;

    // ✅ artık küsuratı koruyarak 2 basamak döndürür
