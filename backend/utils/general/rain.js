// Admin panelinden yönetilen rain / tip limitleri (ChatSettings koleksiyonu).
// Lazy require: controllers/general/chat ile döngüsel bağımlılığı önler.
const generalCheckRainConfigAmount = (amount, kind) => {
    let config = null;
    try {
        config = require('../../controllers/general/chat').generalChatGetConfig();
    } catch(err) { return; }

    const section = kind === 'tip' ? config?.tip : config?.rain;
    if(!section) { return; }

    if(section.enabled === false) {
        throw new Error(kind === 'tip' ? 'Bahşiş özelliği şu anda kapalı.' : 'Rain özelliği şu anda kapalı.');
    }

    const value = Math.floor(amount) / 1000;

    if(section.minAmount > 0 && value < section.minAmount) {
        throw new Error(`Minimum tutar ${section.minAmount}.`);
    }
    if(section.maxAmount > 0 && value > section.maxAmount) {
        throw new Error(`Maksimum tutar ${section.maxAmount}.`);
    }
}
...
const generalCheckSendRainCreateData = (data) => {
    ...
    generalCheckRainConfigAmount(data.amount, 'rain');
}
...
const generalCheckSendRainTipData = (data) => {
    ...
    generalCheckRainConfigAmount(data.amount, 'tip');
}
...
const generalCheckSendRainJoinTransactions = (transactionsDatabase) => {
    ...
    if(amountDeposit < 100 * 1000) {
        throw new Error('You need to have aleast ₺100.00 deposited in the past two weeks.');
    }
}
