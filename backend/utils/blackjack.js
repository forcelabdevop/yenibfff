...
const blackjackCheckSendJoinUser = (user, blackjackTable) => {
    if(user.balance < Math.floor(blackjackGetBetAmountMin(blackjackTable.game.type) )) {
...
const blackjackCheckSendBetSeat = (bet, user, blackjackTable, blackjackSeat) => {
    ...
    } else if(blackjackGetBetAmount(bet.amount) <= 0 || [Math.floor(bet.amount.main), Math.floor(bet.amount.sideLeft), Math.floor(bet.amount.sideRight)].some((element) => element !== 0 && element < Math.floor(blackjackGetBetAmountMin(blackjackTable.game.type) )) === true) {
        throw new Error(`You can only bet a min amount of ₺${parseFloat(blackjackGetBetAmountMin(blackjackTable.game.type)).toFixed(2).replace(/\B(?=(\d{3})+(?!\d))/g, ',')} per seat.`);
    } else if(Math.floor((blackjackSeat.bet === null ? 0 : blackjackGetBetAmount(blackjackSeat.bet.amount)) + blackjackGetBetAmount(bet.amount)) > Math.floor(blackjackGetBetAmountMax(blackjackTable.game.type) )) {
        throw new Error(`You can only bet a total max amount of ₺${parseFloat(blackjackGetBetAmountMax(blackjackTable.game.type)).toFixed(2).replace(/\B(?=(\d{3})+(?!\d))/g, ',')} per seat.`);
...
const blackjackGetBetAmount = (amount) => {
    return parseFloat((amount.main + amount.sideLeft + amount.sideRight).toFixed(2));
}
