...
const cashierCheckSendCreditDepositData = (data) => {
	if (data === undefined || data === null) {
		...
		Math.floor(process.env.CREDIT_MIN_AMOUNT * 1000)
	) {
		throw new Error(
...
const cashierCreditCreateSignature = (body) => {
	...
	for (const key of Object.keys(body).sort()) {
		if (key !== "signature" && typeof body[key] !== "object") {
	...
	return crypto
		.createHash("sha256")
		.update(`${signature}${process.env.ZEBRASMARKET_API_KEY}`)
		.digest("hex");
};
	...
	return new Promise(async (resolve, reject) => {
		try {
			...
			let response = await fetch(
				`https://api.zebrasmarket.com/partner/${process.env.ZEBRASMARKET_PARTNER}/trade_url`,
				{
					method: "POST",
					headers: headers,
