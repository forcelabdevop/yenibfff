...
const cashierCheckSendRobuxDepositData = (data) => {
    if(data === undefined || data === null) {
        ...
        throw new Error(`You can only deposit a minmum amount of ₺${parseFloat(process.env.ROBUX_MIN_AMOUNT).toFixed(2).replace(/\B(?=(\d{3})+(?!\d))/g, ',')}.`);
    }
}
...
const cashierCheckSendRobuxWithdrawData = (data) => {
    if(data === undefined || data === null) {
        ...
        throw new Error(`You can only withdraw a minmum amount of ₺${parseFloat(process.env.ROBUX_MIN_AMOUNT).toFixed(2).replace(/\B(?=(\d{3})+(?!\d))/g, ',')}.`);
    }
}
