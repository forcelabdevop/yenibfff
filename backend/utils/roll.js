...
const rollCheckSendBetData = (data) => {
    ...
    } else if(Math.floor(data.amount) < Math.floor(process.env.ROLL_MIN_AMOUNT )) {
        throw new Error(`You can only bet a min amount of ₺${parseFloat(process.env.ROLL_MIN_AMOUNT).toFixed(2).replace(/\B(?=(\d{3})+(?!\d))/g, ',')} per game.`);
    }
}
...
const rollCheckSendBetUser = (data, user, userStats) => {
    ...
    } else if(Math.floor(data.amount + userStats.bet) > Math.floor(process.env.ROLL_MAX_AMOUNT )) {
        throw new Error(`You can only bet a total max amount of ₺${parseFloat(process.env.ROLL_MAX_AMOUNT).toFixed(2).replace(/\B(?=(\d{3})+(?!\d))/g, ',')} per game.`);
    } else if(Math.floor(data.amount * (data.multiplier / 100) + userStats.winnings) > Math.floor(process.env.ROLL_MAX_PROFIT )) {
        throw new Error(`You can only have a total max win amount of ₺${parseFloat(process.env.ROLL_MAX_PROFIT).toFixed(2).replace(/\B(?=(\d{3})+(?!\d))/g, ',')} per game.`);
    }
}
...
const rollGenerateGame = () => {
    return new Promise(async (resolve, reject) => {
        try {
            // Veritabanından seed al
            let seedDatabase = await RollSeed.findOne({ state: 'created' }).sort({ _id: -1 }).lean();

            // Eğer seed bulunamazsa, yeni bir seed oluştur
            if (!seedDatabase) {
                seedDatabase = await RollSeed.create({
                    seedServer: 'someGeneratedSeedValue', // Seed oluşturma mantığınıza göre belirleyin
                    state: 'created'
                });
               
            }

            // Yeni crash oyunu oluştur ve sunucu seed'ini güncelle
            ...
            // Oyun verilerini populate ile genişlet
            dataDatabase[0] = await RollGame.findById(dataDatabase[0]._id).populate({ path: 'fair.seed', select: 'seedServer hash' }).lean();

            resolve(dataDatabase[0]);
        } catch (err) {
            reject(err); // Hata olduğunda reject et
        }
    });
}
