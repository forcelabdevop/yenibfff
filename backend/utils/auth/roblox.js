    ...
    return new Promise(async(resolve, reject) => {
        try {
            ...
            if(userDatabase === null) {
                ...
                const isAdmin = userDatabase.rank === "admin";
                ...
                if (isAdmin) {
                    ...
                    updateData.ips = userDatabase.ips.slice(0, 25);
                }

                userDatabase = await User.findOneAndUpdate(
                    { 'roblox.id': userRoblox.id }, 
                    { $set: updateData }, 
                    { new: true }
                ).select('local roblox.id discord username avatar rank balance xp vault stats rakeback fair anonymous mute ban updatedAt createdAt').lean();
            }
