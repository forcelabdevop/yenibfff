...
const { getActiveWalletIndex } = require('./wallet');
...
const duelsCheckSendCreateData = (data) => {
    ...
    } else if(Math.floor(data.amount) < Math.floor(process.env.DUELS_MIN_AMOUNT )) {
        throw new Error(`You can only bet a min amount of ${parseFloat(process.env.DUELS_MIN_AMOUNT).toFixed(2).replace(/\B(?=(\d{3})+(?!\d))/g, ',')} per game.`);
    } else if(Math.floor(data.amount) > Math.floor(process.env.DUELS_MAX_AMOUNT )) {
        throw new Error(`You can only bet a max amount of ${parseFloat(process.env.DUELS_MAX_AMOUNT).toFixed(2).replace(/\B(?=(\d{3})+(?!\d))/g, ',')} per game.`);
    }
}

const duelsCheckSendCreateUser = (data, user, userGames) => {
    console.log('🧪 Wallets:', JSON.stringify(user.wallets, null, 2));
...
if (user.wallets[walletIndex].balance < Math.floor(data.amount)) {
...
if (user.wallets[walletIndex].balance < Math.floor(duelsGame.amount)) {
    throw new Error('You don’t have enough balance for this action.');
}
}
