...
const socketCheckUserSuspension = async (decoded) => {
    const userId = decoded?._id || decoded?.id;
    if (!userId) return;

    const user = await User.findById(userId).select("ban").lean();
    if (isActiveUserSuspension(user)) {
        throw createSocketUserSuspensionError();
    }
};
...
const socketCheckUserData = (user, checkAuth) => {
    ...
    } else if(checkAuth === true && isActiveUserSuspension(user)) {
        throw new Error(getUserSuspensionMessage(user));
    }
}
